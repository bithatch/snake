...
/**
 * Input event codes, copied from linux/include/uapi/linux/input-event-codes.h
 */
public enum InputEventCode
{
	...
	private InputEventCode(int code) {
		this.code = code;
		InputNameMap.nameToCode.put(name(), code);
		InputNameMap.codeToName.put(code, name());
	}
	...
	public static InputEventCode fromCode(int code) {
		String name = InputNameMap.codeToName.get(code);
		if (name == null)
			throw new IllegalArgumentException(String.format("No input event with code %d", code));
		return InputEventCode.valueOf(name);
	}

	static class InputNameMap {

		final static Map<String, Integer> nameToCode = new HashMap<>();
		final static Map<Integer, String> codeToName = new HashMap<>();
	}
}
